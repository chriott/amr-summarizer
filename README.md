...
Next, navigate to ```amr_coref/amr_coref/coref/amr_coref_model.py```.
in line 249, add parameter map_location: ```model_dict = torch.load(os.path.join(model_dir, model_fn), map_location='cpu')```
...
Then run the script with any file from the proxy report section of the AMR 3.0 corpus. We have included one compatible file ```proxy-first-20.txt```:
```
python pipeline_eval.py proxy-first-20.txt
```
