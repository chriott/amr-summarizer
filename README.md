...
To run baseline_eval.py, simply run the script with a compatible file. Files from the proxy report section of the AMR 3.0 corpus, or files in this format, are compatible. We have included one such file, ```proxy-first-20.txt```.

```
python baseline_eval.py proxy-first-20.txt
```
